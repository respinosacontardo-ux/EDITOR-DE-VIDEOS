---
name: video-use
description: Edit any video by conversation using only free, local, open-source tools. Transcribe with local Whisper, cut, color grade, generate overlay animations, burn subtitles — for talking heads, montages, tutorials, travel, interviews. No presets, no menus, no paid APIs, no API keys. Ask questions, confirm the plan, execute, iterate, persist. Production-correctness rules are hard; everything else is artistic freedom.
---

# Video Use (100% free-tools edition)

Adapted from [browser-use/video-use](https://github.com/browser-use/video-use). Every tool in this pipeline is free and runs locally: **faster-whisper** (transcription — replaces ElevenLabs Scribe), **ffmpeg/ffprobe**, **PIL**, **yt-dlp**, and optionally **Manim**, **Remotion**, **HyperFrames** (all open source). Never suggest or require a paid API or an API key.
...
6. **Invent freely.** If the material calls for a technique not described here — split-screen, picture-in-picture, lower-third identity cards, reaction cuts, speed ramps, freeze frames, crossfades, match cuts, L-cuts, J-cuts, whatever — build it. The helpers are ffmpeg and PIL. They can do anything the format supports. Do not wait for permission.
7. **Verify your own output before showing it to the user.** If you wouldn't ship it, don't present it.
8. **Free tools only.** Transcription is local Whisper. Never ask the user for an ElevenLabs key or any other paid service. If a technique seems to need a paid API, find or build the free/local equivalent.
...
7. **Pad every cut edge.** Working window: 30–200ms. Whisper word timestamps drift 50–150ms — padding absorbs the drift. Tighter for fast-paced, looser for cinematic.
8. **Word-level timestamps only.** Never SRT/phrase mode (loses sub-second gap data). `transcribe.py` already runs faster-whisper with `word_timestamps=True`, conditioning off, and a filler-seeding prompt to stay as verbatim as Whisper allows.
9. **Cache transcripts per source.** Never re-transcribe unless the source file itself changed. (Local transcription is free but slow — the cache is about time, not money.)
...
Helpers (`helpers/transcribe.py`, `helpers/render.py`, etc.) live alongside this SKILL.md. Resolve their paths relative to the directory containing this file.

## Helpers

- **`transcribe.py <video>`** — single-file local Whisper (faster-whisper) call. `--model <size>` and `--language <code>` optional. Word-level timestamps, Scribe-compatible JSON output. Cached.
- **`transcribe_batch.py <videos_dir>`** — batch transcription; loads the model once and processes files sequentially (faster-whisper already uses all CPU cores per file). Use for multi-take.
...
- **Example cut padding** (from the original launch video): 50ms before the first kept word, 80ms after the last. Tighter for montage energy, looser for documentary. Stay in the 30–200ms working window (Hard Rule 7). With Whisper, lean toward the wider end — its timestamps drift a bit more than a hosted ASR.
- **Whisper caveat:** it doesn't tag audio events like `(laughs)` — spot those from the waveform (`timeline_view.py`) and from context in the transcript instead.
...
Note: local Whisper doesn't diarize, so everything is `S0`. If speaker labels matter (interviews), ask the user who speaks when, or infer from content and note it in `project.md`.
...
**`bold-overlay`** — short-form tech launch, fast-paced social. 2-word chunks, UPPERCASE, break on punctuation, Helvetica 18 Bold, white-on-outline, `MarginV=35`. `render.py` ships with this as `SUB_FORCE_STYLE`. (On Linux, substitute a free font that's actually installed — e.g. `DejaVu Sans` or `Liberation Sans` — Helvetica usually isn't there.)
...
**Tool options** (all free and open source):
...
- Font: a bold monospace — `Menlo.ttc` (index 1) on macOS, `/usr/share/fonts/truetype/dejavu/DejaVuSansMono-Bold.ttf` on Linux
...
- **Whisper SRT / phrase-level output.** Loses sub-second gap data. Always word-level (`transcribe.py` already does this).
- **Using `tiny` for final transcripts.** Fine for a quick draft; final cuts deserve `small` or better — timestamp quality drives cut quality.
- **Suggesting paid APIs.** ElevenLabs, AssemblyAI, Deepgram, etc. are off the table. Local Whisper is the transcription engine, full stop.
