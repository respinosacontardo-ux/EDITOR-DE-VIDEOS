...
def col(w,active):
    if active: return YELA
    return BLUEA if disp(w) in KEY else WHITE
def ass_t(s):
...
for ci,ch in enumerate(chunks):
    ...
    for wi,w in enumerate(ch):
        ...
        for wj,w2 in enumerate(ch):
            c=col(w2['w'], wj==wi)
            sc=r'\fscx116\fscy116' if wj==wi else r'\fscx100\fscy100'
            parts.append('{'+sc+c+'}'+disp(w2['w']))
        pre=r'{\frz%.1f}'%ang
...
"""
open('subs_reel8.ass','w').write(hdr+''.join(f'Dialogue: 0,{ass_t(a)},{ass_t(b)},Sub,,0,0,0,,{t}\n' for a,b,t in events))
print('ass v2:',len(events),'eventos')

B='/home/user/EDITOR-DE-VIDEOS/edit/animations'
CL='/home/user/EDITOR-DE-VIDEOS/edit/clips_reel8'
slots=[('slot_hook',0.12,2.70),('slot_phone',2.45,3.85),('slot_247',6.55,2.90),
       ('slot_auto',10.15,2.90),('slot_pub8',15.40,2.20),('slot_agency8',17.70,2.40),
...
inputs+=['-framerate','30','-i',f'{B}/slot_flash/frames/%04d.png']
...
for k,tc in enumerate(cuts):
    ...
    fp.append(f"{cur}[ff{k}]overlay=enable='between(t,{tc},{tc+0.17})'[vf{k}]")
    cur=f'[vf{k}]'
bi=fi+1
fp.append(f"{cur}[{bi}:v]overlay=x='-1080+1080*(t/{total:.2f})':y=1908:eof_action=pass[vb]")
fp.append("[vb]ass=subs_reel8.ass[outv]")
cmd=['ffmpeg','-y','-v','error',*inputs,'-filter_complex',';'.join(fp),
     '-map','[outv]','-map','0:a','-c:v','libx264','-preset','fast','-crf','18',
     '-pix_fmt','yuv420p','-c:a','copy','-movflags','+faststart','prenorm_reel8.mp4']
subprocess.run(cmd,check=True)
print('composite8 ok')

meas=subprocess.run(['ffmpeg','-y','-hide_banner','-nostats','-i','prenorm_reel8.mp4',
...
subprocess.run(['ffmpeg','-y','-v','error','-i','prenorm_reel8.mp4','-c:v','copy','-af',f,
  '-c:a','aac','-b:a','192k','-ar','48000','-movflags','+faststart',
  '/home/user/EDITOR-DE-VIDEOS/entregas/REEL-8-SISTEMA-CLIENTES-V2.mp4'],check=True)
print('final8 ok')
