...
/* ---------- Instagram: vertical phone with Reel and pulsing engagement ---------- */
export function InstagramMockup() {
  return (
    <div className="flex justify-center">
      <div
        className="relative w-52 md:w-60 rounded-[2.2rem] border-4 border-white/15 bg-black overflow-hidden shadow-2xl"
        style={{ boxShadow: '0 0 90px -20px #E1306C66' }}
      >
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-20 h-5 bg-black rounded-b-2xl z-10 border-x border-b border-white/10" />
        <div className="aspect-[9/17] relative bg-gradient-to-b from-[#24101c] via-[#1a1028] to-[#2e0f1e] p-3 flex flex-col justify-end">
          <div className="absolute top-8 left-0 right-0 flex justify-center gap-4 text-[10px] text-gray-400">
            <span className="text-white font-semibold border-b border-white pb-0.5">Reels</span>
            <span>Siguiendo</span>
          </div>
          <motion.div
            className="absolute inset-x-6 top-16 bottom-24 rounded-xl bg-gradient-to-br from-pink-500/20 to-amber-400/20 border border-white/10 flex items-center justify-center"
            animate={{ scale: [1, 1.02, 1] }}
            transition={{ duration: 3, repeat: Infinity }}
          >
            <InstagramIcon className="w-12 h-12 text-white/60" />
          </motion.div>
          <div className="absolute right-3 bottom-24 flex flex-col gap-4 items-center">
            {[
              { label: '95.3K', delay: 0 },
              { label: '3.1K', delay: 0.5 },
              { label: '1.2K', delay: 1 },
            ].map(({ label, delay }) => (
              <motion.div
                key={label}
                ...
                transition={{ duration: 1.6, repeat: Infinity, delay }}
              >
                <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-pink-300 text-sm">
                  ♥
                </div>
                <span className="text-[9px] text-white">{label}</span>
              </motion.div>
          ...
          <div className="relative space-y-1.5 pb-3 pr-14">
            <div className="flex items-center gap-1.5">
              <div className="w-5 h-5 rounded-full bg-gradient-to-br from-pink-500 to-amber-400 border border-white/20" />
              <span className="text-[10px] text-white font-medium">@tu_negocio</span>
            </div>
            <Bar w="w-3/4" color="rgba(255,255,255,0.25)" />
            <Bar w="w-1/2" color="rgba(255,255,255,0.15)" />
            <div className="flex items-center gap-1.5 text-[9px] text-pink-300">
